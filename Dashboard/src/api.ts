...
const localSupportApi =
  typeof window !== "undefined" &&
  ["localhost", "127.0.0.1", "0.0.0.0"].includes(window.location.hostname)
    ? `http://${window.location.hostname}:4000`
    : "https://triptual-api.onrender.com";
export const SUPPORT_API_BASE_URL = (
  ((import.meta as any).env?.VITE_SUPPORT_API_URL as string | undefined) ??
  localSupportApi
).replace(/\/$/, "");
if (!API_BASE_URL) {
  console.warn(
...
export async function logout() {
  await request("/api/auth/logout", { method: "POST" }, false).catch(
    () => undefined,
  );
  setAccessToken(null);
}
...
export const getUsersList = () =>
  request<{ users: MockUser[]; total: number }>("/api/users");
...
export const getAudienceCount = (audience: string) =>
  request<AudienceCountResponse>(
    `/api/notifications/audience-count?audience=${encodeURIComponent(audience)}`,
  );
...
export const sendBroadcastNotification = (payload: BroadcastPayload) =>
  request<{
    success: boolean;
    message: string;
    stats: BroadcastHistoryItem["stats"];
...
export const getBroadcastHistoryLog = () =>
  request<{ success: boolean; broadcasts: BroadcastHistoryItem[] }>(
...
export const createTourPackage = (payload: any) =>
  request<{ success: boolean; message: string; package: any }>(
    "/api/packages",
    {
      method: "POST",
      body: JSON.stringify(payload),
...
export const updateTourPackageStatus = (id: string, status: string) =>
  request<{ success: boolean; message: string; package: any }>(
    `/api/packages/${encodeURIComponent(id)}/status`,
    {
      method: "PATCH",
      body: JSON.stringify({ status }),
    },
...
export async function getTicketDetail(
  ...
  return request<TicketDetailResponse>(
...
export async function updateTicketStatus(
  ...
  const response = await request<{ success: boolean; ticket: SupportTicket }>(
    `/api/tickets/${encodeURIComponent(ticketNumber)}/status`,
    { method: "PATCH", body: JSON.stringify({ status }) },
  );
  return { success: response.success, ticket: response.ticket };
}
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.error ||
        ...
        "Failed to dispatch ticket message",
    );
  }
